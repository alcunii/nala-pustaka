...
async function test() {
  console.log('Testing OPTIMIZED query (without full_text)...\n');
  console.time('Query');
  const { data, error } = await supabase.from('manuscripts').select('id, title, author, year, description, slug, thumbnail_url, source_url, is_pinned, display_order, created_at, updated_at');
