...
function ChatPanel({ manuscript }) {
  ...
  // Fungsi RAG: Memanggil Backend API untuk grounded chat
  const getGroundedAiResponse = async (userQuery, manuscriptData, conversationHistory = []) => {
    // Use ragApi to call backend service
    // This hides the API key and system prompts on the server
    const { ragApi } = await import('./lib/ragApi');
    ...
    return await ragApi.chatGrounded(userQuery, manuscriptData, formattedHistory);
  };
